...
namespace Plugin\DanhmucMetaSEO\Controller\Admin\Product;
...
class CsvImportController extends AbstractCsvImportController
{
            ...
            if ($form->isValid()) {
                ...
                if (!empty($formFile)) {
                    ...
                    foreach ($data as $row) {
                        ...
                        if (!isset($row[$headerByKey['cseo_title']]) || StringUtil::isBlank($row[$headerByKey['cseo_title']])) {
                            $this->addErrors(($data->key() + 1).'行目のSEOカテゴリ名が設定されていません。');

                            return $this->renderWithError($form, $headers);
                        } else {
                            $Category->setCseoTitle(StringUtil::trimAll($row[$headerByKey['cseo_title']]));
                        }
                        
                        if (!isset($row[$headerByKey['cseo_keywords']]) || StringUtil::isBlank($row[$headerByKey['cseo_keywords']])) {
                            $this->addErrors(($data->key() + 1).'行目のSEOカテゴリディスクリプションが設定されていません。');

                            return $this->renderWithError($form, $headers);
                        } else {
                            $Category->setCseoKeywords(StringUtil::trimAll($row[$headerByKey['cseo_keywords']]));
                        }
    ...
    protected function getCategoryCsvHeader()
    {
        return [
            ...
            //[SEO]タイトルの追加
            trans('admin.product.category_csv.cseo_title_col') => [
                'id' => 'cseo_title',
                'description' => 'admin.product.category_csv.cseo_title_flag_description',
                'required' => false,
            ],
            
            //[SEO]キーワードの追加
            trans('admin.product.category_csv.cseo_keywords_col') => [
                'id' => 'cseo_keywords',
                'description' => 'admin.product.category_csv.cseo_keywords_flag_description',
                'required' => false,
            ],
        ];
    }
